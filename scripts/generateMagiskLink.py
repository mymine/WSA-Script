...
if magisk_branch == "vvb2060":
    try:
        magisk_link = json.loads(requests.get(
            f"https://install.appcenter.ms/api/v0.1/apps/vvb2060/magisk/distribution_groups/public/releases/latest?is_install_page=true", headers=headers, timeout=60).content)['download_url']
    ...
    try:
        magisk_link = json.loads(requests.get(
            f"https://github.com/{magisk_branch}/magisk-files/raw/master/{magisk_ver}.json", timeout=60).content)['magisk']['link']
        download_files[f"magisk-{magisk_ver}.zip"] = magisk_link
    except Exception:
        print("Failed to fetch from GitHub API, fallbacking to jsdelivr...")
        magisk_link = json.loads(requests.get(
            f"https://fastly.jsdelivr.net/gh/topjohnwu/magisk-files@master/{magisk_ver}.json", timeout=60).content)['magisk']['link']
        download_files[f"magisk-{magisk_ver}.zip"] = magisk_link
res = requests.get(
    f"https://api.github.com/repos/LSPosed/WSA-Addon/releases/latest", auth=github_auth, timeout=60)
